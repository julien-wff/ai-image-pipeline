...
async def upload_image(
    ...
    return ImageResponse.from_model(db_image)
...
async def list_images(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    return [ImageResponse.from_model(img) for img in images]
...
async def delete_image(image_id: int, db: Session = Depends(get_db)):
    ...
    # Delete files
    try:
        if os.path.exists(image.upload_path):
            os.remove(image.upload_path)
